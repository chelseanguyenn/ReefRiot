...
function normalizeTrashType(value) {
  ...
  if (
    text.includes("net") ||
    text.includes("ghostnet") ||
    text.includes("fishingnet")
  ) {
    return "net";
  }

  if (text.includes("straw")) {
    return "straw";
  }

  if (text.includes("bottle") || text.includes("plasticbottle")) {
    return "bottle";
  }

  if (text.includes("bag") || text.includes("plasticbag")) {
    return "bag";
  }
...
function getEncounterTypeFromEvent(event) {
  ...
  for (const value of possibleValues) {
    const normalized = normalizeTrashType(value);
...
function getRemovedTrash(beforeTrash = [], afterTrash = []) {
  const afterIds = new Set(afterTrash.map((item) => item.id));
  return beforeTrash.filter((item) => !afterIds.has(item.id));
}

function countRemovedTrashByType(removedTrash = []) {
  const counts = {
    net: 0,
    straw: 0,
    bottle: 0,
  ...
  for (const item of removedTrash) {
    const type =
      normalizeTrashType(item.kind) ||
      normalizeTrashType(item.type) ||
      normalizeTrashType(item.id);
...
function combineEncounterCounts(a, b) {
  return {
    net: (a?.net || 0) + (b?.net || 0),
    straw: (a?.straw || 0) + (b?.straw || 0),
    bottle: (a?.bottle || 0) + (b?.bottle || 0),
    bag: (a?.bag || 0) + (b?.bag || 0),
  };
}

function countEncounterIncrements(
  collisionEvents = [],
  beforeTrash = [],
  afterTrash = []
) {
  ...
  for (const event of collisionEvents) {
    const type = getEncounterTypeFromEvent(event);
    if (type) {
      fromEvents[type] += 1;
    }
  }

  const removedTrash = getRemovedTrash(beforeTrash, afterTrash);
  const fromRemovedTrash = countRemovedTrashByType(removedTrash);

  return combineEncounterCounts(fromEvents, fromRemovedTrash);
}
...
export default function UseGameState() {
  ...
  const nodesRef = useRef(createInitialState().nodes);

  const handleInteract = useCallback(() => {
    setState((prev) => {
      const { nearNodeId, nodes, objectives, nodesComplete } = prev;
      if (!nearNodeId || prev.gameOver) return prev;
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setState((prev) => {
        ...
        if (spd > MAX_SPEED) {
          nvx = (nvx / spd) * MAX_SPEED;
          nvy = (nvy / spd) * MAX_SPEED;
        }

        const nx = Math.max(
          SHARK_SIZE / 2,
          Math.min(ARENA_WIDTH - SHARK_SIZE / 2, prev.sharkX + nvx)
        );
        const ny = Math.max(
          SHARK_SIZE / 2,
          Math.min(ARENA_HEIGHT - SHARK_SIZE / 2, prev.sharkY + nvy)
        );
        ...
        for (const node of prev.nodes) {
          ...
          if (d < NODE_INTERACT_RADIUS && d < nearDist) {
            nearDist = d;
            nearNodeId = node.id;
          }
        }
        ...
        const updatedEnemies = prev.enemies.map((enemy) => {
          ...
          if (espd > ENEMY_SPEED * 1.5) {
            evx = (evx / espd) * ENEMY_SPEED * 1.5;
            evy = (evy / espd) * ENEMY_SPEED * 1.5;
          }

          const ex = enemy.x + evx;
          const ey = enemy.y + evy;
        ...
        const encounterAdds = countEncounterIncrements(
          collisionEvents,
          currentTrash,
          updatedTrash
        );
        ...
        return {
          ...
          sharkX: nx,
          sharkY: ny,
          sharkVx: gameOver ? 0 : nvx,
          sharkVy: gameOver ? 0 : nvy,
          sharkFacing: nvx > 0.1 ? 1 : nvx < -0.1 ? -1 : prev.sharkFacing,
          sharkState: gameOver
            ? "idle"
            : prev.nodesComplete === 3
            ? "restored"
            : nearNodeId
            ? "disrupting"
            : moving
            ...
            : "idle",
          sharkAngle: angle,
          nearNodeId: gameOver ? null : nearNodeId,
          ...
          encounters: {
            net: prev.encounters.net + encounterAdds.net,
            straw: prev.encounters.straw + encounterAdds.straw,
            bottle: prev.encounters.bottle + encounterAdds.bottle,
            bag: prev.encounters.bag + encounterAdds.bag,
          },
        };
      });
  ...
  const reset = useCallback(() => {
    ...
    sharkPosRef.current = { x: freshState.sharkX, y: freshState.sharkY };
